...
const performanceStats = ref({
    topPerformer: { name: 'สมชาย ใจดี', totalWeight: 1250, totalDeals: 45 },
    topService: { name: 'วิชัย รักงาน', rating: 4.9, reviews: 32 }
});

const quantityChartData = ref({
    labels: ['สมชาย', 'วิชัย', 'มานะ', 'ปิติ', 'ชูใจ'],
    datasets: [{
        label: 'ปริมาณรับซื้อ (กก.)',
        backgroundColor: '#3b82f6',
        data: [1250, 980, 850, 600, 450]
    }]
});
...
const ratingChartData = ref({
    labels: ['การบริการ', 'ความสุภาพ', 'ความเร็ว', 'ความแม่นยำ', 'การแก้ปัญหา'],
    datasets: [{
        label: 'คะแนนเฉลี่ยทีมงาน',
        backgroundColor: 'rgba(34, 197, 94, 0.2)',
        borderColor: '#22c55e',
        pointBackgroundColor: '#22c55e',
        pointBorderColor: '#fff',
        data: [4.8, 4.5, 4.2, 4.9, 4.6]
    }]
});
    ...
    <div class="space-y-8">
        ...
        <div>
            <h3 class="text-lg font-bold mb-4 flex items-center gap-2">
                ...
                ประสิทธิภาพพนักงาน (Performance)
            </h3>

            <!-- Top Performers Cards -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <!-- Top Recycler -->
                <div class="card bg-base-100 shadow-sm border border-blue-100">
                    <div class="card-body p-4 flex flex-row items-center gap-4">
                        <div class="avatar placeholder">
                            <div class="bg-blue-100 text-blue-600 rounded-full w-12 h-12">
                                <span class="text-xl">🏆</span>
                            </div>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 font-semibold uppercase">Top Recycler</div>
                            <div class="font-bold text-lg">{{ performanceStats.topPerformer.name }}</div>
                            <div class="text-xs text-blue-600 font-medium">รับซื้อรวม {{
                                performanceStats.topPerformer.totalWeight }} กก. ({{
                                    performanceStats.topPerformer.totalDeals }} รายการ)</div>
                        </div>
                    </div>
                </div>

                <!-- Top Service -->
                <div class="card bg-base-100 shadow-sm border border-green-100">
                    <div class="card-body p-4 flex flex-row items-center gap-4">
                        <div class="avatar placeholder">
                        ...
                        <div>
                            <div class="text-xs text-gray-500 font-semibold uppercase">Top Service</div>
                            <div class="font-bold text-lg">{{ performanceStats.topService.name }}</div>
                            <div class="text-xs text-green-600 font-medium">คะแนนเฉลี่ย {{
                                performanceStats.topService.rating }} / 5.0</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card bg-base-100 shadow-sm border p-4">
                    <h4 class="font-semibold text-sm mb-4 text-center">ปริมาณการรับซื้อรายบุคคล (เดือนนี้)</h4>
                    <div class="h-64">
                ...
                <!-- Rating Radar Chart -->
                <div class="card bg-base-100 shadow-sm border p-4">
                    <h4 class="font-semibold text-sm mb-4 text-center">คุณภาพการบริการทีมงาน</h4>
                    <div class="h-64">
                        <Radar :data="ratingChartData" :options="ratingChartOptions" />
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-box">
                    ...
                    <form @submit.prevent="handleSubmit">
                        ...
                        <div class="form-control mb-4">
                            ...
                            <input type="text" v-model="formData.username" class="input input-bordered w-full"
                                required />
                        </div>
                        ...
                        <div class="form-control mb-6">
                            ...
                            <label class="label" v-if="!isEditing">
                                <span class="label-text-alt text-gray-500">รหัสผ่านต้องมีความยาวอย่างน้อย 6
                                    ตัวอักษร</span>
                            </label>
                        </div>
